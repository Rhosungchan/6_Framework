...
function loginValidate(){
    ...
    if(memberPw.value.trim().length == 0 ){
        // 비밀번호 양쪽 공백을 제거한 후 길이가 0인 경우 == 비밀번호 미작성    
...
selectEmail.addEventListener("click",(e)=>{
    ...
    $.ajax({
        url : "/selectEmail", // (controller에서 GetMapping의 주소 값)
        data : {"email" : inputEmail.value },
        type : "POST",
        dataType : "JSON", // 응답 데이터의 형식이 JSON이다. -> 자동으로 JS객체 변환
        success : (member) => {
            console.log(member);
            ...
            if(member == null){ // 일치 X
                                ...
                                // selectEamil의 다음 요소로 추가(.after(요소))
