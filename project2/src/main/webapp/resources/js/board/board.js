...
boardLike.addEventListener("click", e => {
    ...
    // 로그인한 상태이면서 + 좋아요 상태인 경우 
        ...
        $.ajax({
            ...
            success: (result) => {
                if (result > 0) { // 성공
                    e.target.classList.remove('fa-solid');      // 빈하트 클래스 삭제
                    e.target.classList.add('fa-regular');      // 채워진 하트 클래스 추가
...
updateBtn.addEventListener("click", () => {
    ...
    // 상세 조회 :  /board/{boardCode}/{boardNo}/update    ?cp=10
