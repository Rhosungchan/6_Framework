...
document.getElementById("signUp-frm").addEventListener("submit", function (event) {
    ...
    // for in 구문 : 객체에 key값을 순서대로 접근하는 반복문
    // [작성법]
    // for(let 변수명 in 객체명)
    // 변수명 == key
    // -> 객체에서 순선대로 key를 하나씩 꺼내 왼쪽 변수에 저장 
...
// input 이벤트 : input 태그에 입력이 되었을 경우 (모든 입력 방식 적용)
        ...
        $.ajax({
            ...
            success: (result) => { // 비동기 통신을 성공해서 응답을 받았을 때.
                ...
                if (result == 0) { // 중복 아님
                    ...
                    emailMessage.innerText = "사용 불가능한 이메일입니다.";
...
memberPw.addEventListener("input", function () {
    ...
    if (regEx.test(memberPw.value)) { // 유효한 비밀번호
        ...
        if (memberPwConfirm.value.trim().length == 0) { // 유효한 비밀번호 + 확인 작성 X 
